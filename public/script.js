...
const API_URL =
  "https://icy-cherry-1d53treats-api.clarencecreations.workers.dev/answers";
...
/* Optional — works even if this does not exist in your HTML */
...
async function saveQuizSubmission() {
  ...
  setSaveStatus("Saving your response...", "saving");

  const payload = {
    name: playerName,
    score,
    answers,
    wantedDate: dateAnswerYes,
    preferredDate: bookingDetails?.preferredDate || "",
    preferredTime: bookingDetails?.preferredTime || "",
    note: bookingDetails?.dateIdea || ""
  };

  submissionPromise = fetch(API_URL, {
    method: "POST",
    headers: {
    ...
    .then(async (response) => {
      ...
      setSaveStatus("Response saved privately 💗", "saved");
      return true;
    })
    .catch((error) => {
      console.error("Could not save quiz submission:", error);
      setSaveStatus("Could not save this response.", "error");
      return false;
    })
    .finally(() => {
      submissionPromise = null;
